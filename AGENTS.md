...
- Post-task transfer pass: translation attempts and practice sessions share one card pass over the notes they produced — `src/lib/transfer-queue.ts` (queue rules), `components/review/TransferStage.svelte` (card UI), `server/transfer.ts` (listing, ownership, rating). Every note is created due at the learner's next local day (`insertNotes` requires `availableFrom`), so a task never refills the same day's `/review` queue; the pass rates those not-yet-due cards through `rateNote(..., { outOfBand: true })`, which never pulls them into today.
- Streak: rules are pure functions in `src/lib/streak.ts` (always take `today` as an argument, never read a clock); `server/streak.ts` owns the only writes and is called from the transaction that already claimed each completion. A day counts when a quest completes **and** the account-wide review queue is observed empty; settlement of missed days is derived on read and materialised on write, so loaders never write. `/streak-lab` is the dev harness (states, animations, rule simulation, real writes plus cookie day travel).
...
- For big changes, this repository follows an design-implement-writeup workflow. Design and writeup files are at `docs/design/` and `docs/writeup/`. A design should present "what to do", which describes features to implement and bugs to fix, and goes through important technical decisions and standards. After implementation is finished, writeup records "what was done" in a **human readable** (without a lot of confusing terms or names in the code) and **concise** format, so collaborators can refer to them and quickly understand what has changed so far in the codebase when they move on to new changes.
    - Design's frontmatter: `title`, `type` (can have multiple: bug / feature / ux / performance / accessibility / security / tech-debt / test), `status` (needs-approval -> wip -> done; done can only be marked by human),
    - Writeup's frontmatter: `title`, `related-design` (a path of the design from project root)
    - Filename example: "2026-09-18-some-title.md"
