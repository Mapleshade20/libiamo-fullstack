...
- Quest Hall owns both scheduled practice quests and the translation-template catalog. Scheduled quests use the editorial `QuestEdition`/`QuestBrief` master-detail interface and link directly to session or feedback; the separate `TranslationIndex` appears below This Week, loads the catalog once, and keeps creation-month switching in client state without URL navigation. `/translate` plus `/task` roots redirect to `/`; their `[id]` routes remain the task detail/workflow entry points.
