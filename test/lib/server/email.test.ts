...
describe("account email HTML", () => {
	it.each([
		["verification", emailVerificationHtml, "learner@example.com", "https://example.com/verify"],
		["password reset", resetPasswordHtml, "user@test.com", "https://example.com/reset"],
	] as const)("renders a complete %s document with its recipient and action URL", (_kind, renderHtml, email, url) => {
		const html = renderHtml(email, url);
