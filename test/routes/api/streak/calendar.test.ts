...
describe("private bounded calendar endpoint", () => {
	...
	it("scopes to the session user, uses local day and forbids shared caching", async () => {
		mocks.getCalendar.mockResolvedValue({ days: [{ day: "2026-09-30", state: "lit" }], since: "2026-09-01" });
		const response = await get("2026-09");
		expect(mocks.getCalendar).toHaveBeenCalledExactlyOnceWith("learner", "2026-09", "2026-09-30");
