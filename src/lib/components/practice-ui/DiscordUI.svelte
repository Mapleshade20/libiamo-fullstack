<script lang="ts">
	...
	const i18n = {
		en: {
			...
			unavailableFeature:
				"In the immersive learning context, this feature is unavailable.",
		},
		es: {
			...
			unavailableFeature:
				"En el contexto de aprendizaje inmersivo, esta función no está disponible.",
		},
		fr: {
			...
			unavailableFeature:
				"Dans le contexte d'apprentissage immersif, cette fonctionnalité n'est pas disponible.",
		},
		ja: {
		...
		if (existingSession && existingSession.id !== lastLoadedSessionId) {
			...
			messages =
				existingSession.messages?.map((m: any) => ({
					...
					avatarColor:
						m.role !== "user" ? agentUser.color : undefined,
	...
	async function handleSend() {
		if (!inputText.trim() || isSubmitting || isCompleted || isInitializing)
			return;
	...
	function handleKeydown(e: KeyboardEvent) {
		if (showMentionMenu && filteredMentionUsers.length > 0) {
			...
			if (e.key === "ArrowUp") {
				e.preventDefault();
				mentionIndex =
					(mentionIndex - 1 + filteredMentionUsers.length) %
...
>
	...
	<div
		class="flex w-[72px] flex-col items-center gap-2 overflow-y-auto bg-[#1E1F22] py-3 hide-scrollbar"
	>
		<a
			href={`/task/${taskId}`}
		...
		<button
			type="button"
			class="relative flex h-12 w-12 items-center justify-center rounded-[16px] bg-[#5865F2] text-white transition-all"
		>
			<div
				class="absolute -left-3 top-2 h-8 w-1 rounded-r-full bg-white"
			></div>
	...
	<div class="flex w-60 flex-col bg-[#2B2D31]">
		...
		<div class="flex-1 overflow-y-auto p-2 hide-scrollbar">
			...
			<button
				type="button"
				class="flex w-full items-center gap-1.5 rounded bg-[#404249] px-2 py-1.5 text-[#DBDEE1]"
			>
		...
		<div
			class="flex h-[52px] items-center justify-between bg-[#232428] px-2"
		>
			<button
				type="button"
			...
			>
				<div class="relative h-8 w-8 shrink-0">
					<div
						class="h-full w-full rounded-full bg-[#5865F2] overflow-hidden flex items-center justify-center font-bold text-white"
					>
						{#if avatarUrl}
							<img
								src={avatarUrl}
					...
					<div
						class="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-[2.5px] border-[#232428] bg-[#23A559]"
					></div>
				</div>
				<div class="flex flex-col items-start text-sm truncate">
					<span
						class="font-semibold leading-tight text-white truncate w-full text-left"
						>{userName}</span
					>
			...
			<div class="flex gap-1 text-[#B5BAC1]">
				<button
					type="button"
					class="rounded p-1.5 hover:bg-[#35373C] hover:text-[#DBDEE1]"
					onclick={handleMockAction}><Mic size={18} /></button
				>
				<button
					type="button"
					class="rounded p-1.5 hover:bg-[#35373C] hover:text-[#DBDEE1]"
				...
				<button
					type="button"
					class="rounded p-1.5 hover:bg-[#35373C] hover:text-[#DBDEE1]"
					onclick={handleMockAction}><Settings size={18} /></button
				>
			</div>
		</div>
	</div>
	...
	<div class="flex flex-1 flex-col bg-[#313338] min-w-0">
		<div
			class="flex h-12 shrink-0 items-center justify-between border-b border-[#1F2023] px-4 shadow-sm"
		>
		...
		<div class="flex flex-1 overflow-hidden">
			<div class="flex flex-1 flex-col min-w-0 relative">
				...
				>
					<div class="my-4 mt-auto flex items-center justify-center">
						...
						<span class="px-2 text-xs font-semibold text-[#949BA4]"
							>{getTodayDateString()}</span
						>
						<div class="h-px flex-1 bg-[#404249]"></div>
					</div>
					...
					{#each messages.filter((m) => !m.isHidden) as msg}
						<div
							class="mt-4 flex hover:bg-[#2E3035] p-1 -mx-4 px-4 rounded group"
						>
							<div
							...
							<div class="flex-1">
								...
								<div class="text-[#DBDEE1] whitespace-pre-wrap">
									{msg.text}
								</div>
							</div>
						</div>
					...
					{#if isInitializing || isSubmitting}
						<div
							class="mt-4 flex p-1 -mx-4 px-4 items-center gap-3"
						>
							<div class="flex gap-1">
								<span
									class="w-2 h-2 rounded-full bg-[#80848E] animate-bounce"
								></span>
								<span
									class="w-2 h-2 rounded-full bg-[#80848E] animate-bounce"
					...
					{#if isCompleted && feedback}
						<div
							class="mt-8 rounded-lg border border-[#1E1F22] bg-[#2B2D31] p-6 shadow-lg relative overflow-hidden"
						>
							<div
								class="absolute top-0 left-0 w-1 h-full bg-[#FEE75C]"
							></div>
							<div
								class="flex items-center gap-2 mb-4 text-[#FEE75C]"
							>
								<AlertCircle size={24} />
							...
							<p
								class="text-[#DBDEE1] mb-6 leading-relaxed whitespace-pre-wrap"
							>
								{feedback.content}
							</p>
							...
							<div class="space-y-3">
								<h4
									class="text-sm font-semibold text-[#949BA4] uppercase tracking-wider"
								>
									Objective Results
								</h4>
								{#each feedback.objectiveResults as obj}
									<div
										class="flex items-start justify-between rounded bg-[#1E1F22] p-3"
									>
										<span
											class="text-sm text-[#DBDEE1] pr-4"
											>{obj.text}</span
										>
										<span
				...
				<div class="px-4 pb-6 pt-1 shrink-0 relative">
					{#if showMentionMenu && filteredMentionUsers.length > 0}
						<div
							class="absolute bottom-[100%] left-4 mb-2 w-72 bg-[#2B2D31] border border-[#1E1F22] rounded shadow-xl overflow-hidden z-50"
						>
							<div
								class="px-3 py-2 text-xs font-bold text-[#949BA4] uppercase bg-[#232428]"
							>
								Members
							</div>
							<ul
								class="max-h-60 overflow-y-auto hide-scrollbar py-1"
							>
								{#each filteredMentionUsers as user, i}
									<li class="mx-1">
										...
										>
											...
											<span
												class="text-[#DBDEE1] text-sm font-medium"
												>{user.name}</span
											>
											{#if user.isAgent}
												<span
													class="ml-auto text-[10px] bg-[#5865F2] text-white px-1.5 rounded font-bold uppercase tracking-wide"
													>Bot</span
												>
											{/if}
										</button>
					...
					>
						...
						/>
						<div
							class="flex gap-3 text-[#B5BAC1] relative emoji-container-wrapper"
						>
							<button
								type="button"
								class="transition-colors hover:text-[#DBDEE1]"
								onclick={handleMockAction}
								><Gift size={22} /></button
							>
							<button
							...
							{#if showEmojiPicker}
								...
								>
									<div
										class="px-3 py-2 text-xs font-bold text-[#949BA4] uppercase bg-[#232428] border-b border-[#1E1F22]"
									>
										Common Emojis
									</div>
									<div
										class="p-2 grid grid-cols-8 gap-1 max-h-60 overflow-y-auto hide-scrollbar"
									>
			...
			{#if showMembers}
				<div
					class="w-60 bg-[#2B2D31] shrink-0 flex flex-col hide-scrollbar overflow-y-auto px-2 py-4"
				>
					<h3
						class="px-2 pt-2 pb-1 text-[12px] font-semibold text-[#949BA4] uppercase"
					>
						{t.online} — {onlineUsers.length + 2}
					</h3>

					<div
						class="flex items-center gap-3 px-2 py-1.5 hover:bg-[#35373C] rounded mt-0.5 opacity-100 transition-colors cursor-default"
					>
						<div class="relative h-8 w-8 shrink-0">
							<div
								class="h-full w-full rounded-full bg-[#5865F2] overflow-hidden flex items-center justify-center font-bold text-white"
							...
							<div
								class="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full border-[2.5px] border-[#2B2D31] bg-[#23A559]"
							></div>
						</div>
						<span
							class="text-[#DBDEE1] font-medium text-sm truncate"
							>{userName} (You)</span
						>
					</div>
					...
					>
						<div class="relative h-8 w-8 shrink-0">
							<div
								class="h-full w-full rounded-full {agentUser.color} overflow-hidden flex items-center justify-center font-bold text-white"
							>
						...
						<div class="flex flex-col justify-center min-w-0">
							<div class="flex items-center gap-1">
								...
								>
								<span
									class="text-[9px] bg-[#5865F2] text-white px-1 rounded font-bold uppercase tracking-wide leading-tight"
									>Bot</span
								>
							</div>
							<span class="text-xs text-[#B5BAC1] truncate"
								>{agentUser.status}</span
							>
						</div>
					</button>
					...
					{#each onlineUsers as user}
						...
						>
							<div class="relative h-8 w-8 shrink-0">
								<div
									class="h-full w-full rounded-full {user.color} overflow-hidden flex items-center justify-center font-bold text-white"
								>
					...
					<h3
						class="px-2 pt-6 pb-1 text-[12px] font-semibold text-[#949BA4] uppercase"
					>
					...
					{#each offlineUsers as user}
						...
						>
							...
							<span
								class="text-[#80848E] font-medium text-sm truncate"
								>{user.name}</span
							>
						</button>
					{/each}
