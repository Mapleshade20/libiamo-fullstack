...
function appendAgentMessageFromSendResult(result: SendAttemptResult, clientMessageId: string, retryText: string, agentMessageId = crypto.randomUUID()) {
...
function loadExistingSession(session: any) {
	...
	if ((!selectedInboxId || (activeMailbox === "inbox" && selectedInboxId.startsWith("agent-") && !selectedGeneratedInboxExists)) && visibleAgentMessages.length) {
...
$effect(() => {
	if (limitReached && currentTurns > 0 && !isWaitingRetry && !isAnyMessagePending && !isSubmitting && !isCompleting && !isCompleted && sessionId && !hasAutoCompleted) {
