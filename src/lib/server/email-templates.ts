...
export function emailVerificationHtml(userEmail: string, url: string): string {
	return wrapContent(
		...
		"Welcome to Libiamo!",
		`We're excited to have you on board! Please verify your email address <strong>${userEmail}</strong> to get started with your language learning journey. This link will expire in 24 hours.`,
...
export function resetPasswordHtml(userEmail: string, url: string): string {
	return wrapContent(
		...
		`No worries — it happens to the best of us! Click the button below to set a new password for <strong>${userEmail}</strong>. This link will expire in 1 hour for security.`,
		"Reset Password",
		url,
	);
}
