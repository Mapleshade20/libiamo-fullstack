...
<div class="space-y-8">
	...
	<form method="GET" class="flex flex-wrap gap-3">
		...
		<div class="space-y-1">
			...
			>
				<option value="en" selected={data.filters.language === "en"}
					>English</option
				>
				<option value="es" selected={data.filters.language === "es"}
					>Spanish</option
				>
				<option value="fr" selected={data.filters.language === "fr"}
					>French</option
				>
				<option value="ja" selected={data.filters.language === "ja"}
					>Japanese</option
				>
			</select>
		</div>
				...
				<div class="space-y-1">
					...
					>
						{#each data.activeTemplates as tpl}
							<option value={tpl.id}
								>{tpl.id} — {tpl.titleBase} ({tpl.language.toUpperCase()})</option
							>
						{/each}
					</select>
					{#if form?.errors?.templateId}
						<p class="text-sm text-red-600">
							{form.errors.templateId[0]}
						</p>
					{/if}
				</div>
				<div class="space-y-1">
