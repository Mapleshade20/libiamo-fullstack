...
let targetLanguageTag = $derived(getHtmlLanguageTag(data.task.language));
let detailsHref = $derived(data.detailsPath);
...
<svelte:head><title>{data.task.title} · Draft</title></svelte:head>
