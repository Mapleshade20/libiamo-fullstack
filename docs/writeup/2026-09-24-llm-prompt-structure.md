...
- **Correction Verifier** payload is ordered task translation context → card context → shown hint → revision, and names the shown hint instead of duplicating its text. The translation context is new: it lets `fullyNatural` judge contextual fit. Generation 1 and the Second Draft Verifier are unchanged.
...
- **Hints.** Content and expression hints followed their contracts (a direction in the native language; short target-language fragments). On `gpt-6-luna` the first-turn content hint occasionally came back as bare text (about 1 in 12), so the content hint now uses `chatText`. A follow-up run of 4 situations × 3 (iMessage and Reddit sessions, empty and partial drafts) returned 12/12 plain-text hints with no repair. Early hints bundled several directions and leaked names like "objective 3" and "currentDraft"; after tightening the output rules each hint is one short sentence, e.g. 「Alex 正等你答复：直接确认去 the ridge trail，并敲定周六集合的具体时间和地点，把最终计划落实下来。」
- **Correction Verifier with translation context, 3 revisions × 2 runs, with and without context.** Verdicts were identical. With the context, the feedback cites it directly, e.g. 「“hit you back”“real quick”和“No worries”过于随意，不符合银行正式回复邮件的语气。」
